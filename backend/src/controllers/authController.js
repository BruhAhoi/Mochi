...
export const signUp = async (req, res) => {
  try {
    ...
    //mã hóa password
    const hashedPassword = await bcrypt.hash(password, 10); //slat = 10
    //tạo user mới
    await User.create({
      username,
      hashedPassword,
...
export const signIn = async (req, res) => {
  try {
    ...
    //lấy hashedPassword từ db để so sánh với password từ input
...
export const refreshToken = async (req, res) => {
  try{
    ...
    if(session.expiresAt < new Date()){
      return res.status(403).json({message: "Refresh token expired. Please sign in again."});
    }
